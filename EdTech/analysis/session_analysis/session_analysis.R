...
std_ss_dur = read.csv('../analysis/session_analysis/data/student_ss_dur.csv')
std_ss_dur = subset(std_ss_dur, space_1 != 'nan')
std_ss_dur$space_1 = factor(std_ss_dur$space_1)
library(ggplot2)
g = ggplot(std_ss_dur, aes(space_1, duration)) + geom_boxplot() 
g = g + labs(title = 'Boxplot for session length varied by courses', x = 'Courses',y='Session length (s)')
g
