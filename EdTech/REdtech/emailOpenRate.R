...
gWeekly = gWeekly + xlab('Week') + ylab('Emails opened') + ggtitle('Weekly Emails opened per course')
gWeekly
# daily log-in
gDaily = ggplot(data=dfDay, aes(x=day, y=x, group=course, color=course)) + geom_line()
gDaily = gDaily + xlab('Date') + ylab('Emails opened') + ggtitle('Daily Emails opened per course')
